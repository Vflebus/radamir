...
CREATE OR ALTER FUNCTION new_campaign(VARCHAR(255), TEXT, INT) RETURNS INT AS $$
	INSERT INTO "campaign"("campaign_name", "description", "user_id") 
VALUES($1, $2, $3) RETURNING id;
$$ LANGUAGE SQL STRICT;

CREATE OR ALTER FUNCTION update_campaign(VARCHAR(255), TEXT, INT) RETURNS void AS $$
    UPDATE "campaign" SET "campaign_name" = $1, "description" = $2 WHERE id = $3;
$$ LANGUAGE SQL STRICT;

-- user new & update

CREATE OR ALTER FUNCTION new_user(TEXT, TEXT, TEXT) RETURNS INT AS $$
    INSERT INTO "user"("username", "email", "password")
VALUES($1, $2, $3) RETURNING id;
$$ LANGUAGE SQL STRICT;

CREATE OR ALTER FUNCTION update_user(TEXT, TEXT, TEXT, INT) RETURNS void AS $$
    UPDATE "user" SET "username" = COALESCE($1, "username"), "email" = COALESCE($2, "email"), "password" = COALESCE($3, "password") WHERE id = $4;
$$ LANGUAGE SQL;

-- notes new & update
CREATE OR ALTER FUNCTION new_note(TEXT, TEXT, BOOLEAN, INT, INT, VARCHAR(255)) RETURNS INT AS $$
	INSERT INTO "note"("title", "content", "is_private", "campaign_id", "user_id", "image_url") 
    VALUES($1, $2, $3, $4, $5, $6) RETURNING id;
$$ LANGUAGE SQL STRICT;

CREATE OR ALTER FUNCTION update_note(TEXT, TEXT, BOOLEAN, INT) RETURNS void AS $$
...
CREATE OR ALTER FUNCTION new_wiki(TEXT, VARCHAR(255), TEXT, VARCHAR(255)) RETURNS INT AS $$
	INSERT INTO "wiki"("slug", "title", "type", "full_title")
	VALUES($1, $2, $3, $4) 
	RETURNING id;
$$ LANGUAGE SQL;

CREATE OR ALTER FUNCTION update_wiki(TEXT, VARCHAR(255), TEXT, INT, VARCHAR(255) ) RETURNS void AS $$
...
CREATE OR ALTER FUNCTION update_block(VARCHAR(16), TEXT, INT) RETURNS void AS $$
